...
/* New tables to support Leaves, Recruitment, and Reports features */
...
CREATE TABLE IF NOT EXISTS candidates (
  ...
  job_posting_id INT (11) UNSIGNED NOT NULL,
  status VARCHAR(30) NOT NULL DEFAULT 'Préselection',
...
ALTER TABLE job_postings ADD FOREIGN KEY (department_id) REFERENCES departments(department_id);
ALTER TABLE candidates ADD FOREIGN KEY (job_posting_id) REFERENCES job_postings(job_posting_id);
ALTER TABLE leaves ADD FOREIGN KEY (employee_id) REFERENCES employees(employee_id);

/* Seed minimal sample data */
INSERT INTO job_postings (title, department_id, status, created_at) VALUES
  ('Développeur Full Stack', 60, 'Open', CURRENT_DATE),
  ('Designer UX/UI', 20, 'Open', CURRENT_DATE);

INSERT INTO candidates (name, job_posting_id, status, applied_date, rating) VALUES
  ('Karim Hamdi', 1, 'Entretien', DATE_SUB(CURRENT_DATE, INTERVAL 57 DAY), 4.5),
  ('Hana Mzali', 2, 'Préselection', DATE_SUB(CURRENT_DATE, INTERVAL 54 DAY), 4.0);

INSERT INTO leaves (employee_id, leave_type, start_date, end_date, days, status) VALUES
  (100, 'Congé Annuel', DATE_SUB(CURRENT_DATE, INTERVAL 10 DAY), DATE_SUB(CURRENT_DATE, INTERVAL 1 DAY), 10, 'Approuvé'),
  (201, 'Congé Maladie', DATE_SUB(CURRENT_DATE, INTERVAL 3 DAY), CURRENT_DATE, 3, 'En attente');
